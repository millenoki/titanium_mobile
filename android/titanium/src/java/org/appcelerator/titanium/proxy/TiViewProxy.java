...
@Kroll.proxy(propertyAccessors = {
        ...
        TiC.PROPERTY_BACKGROUND_COLOR, TiC.PROPERTY_BACKGROUND_IMAGE,
        TiC.PROPERTY_BACKGROUND_REPEAT, TiC.PROPERTY_BACKGROUND_SELECTED_COLOR,
        ...
        TiC.PROPERTY_BACKGROUND_PADDING, TiC.PROPERTY_BACKGROUND_GRADIENT,
        // border properties
        TiC.PROPERTY_BORDER_COLOR, TiC.PROPERTY_BORDER_RADIUS,
        TiC.PROPERTY_BORDER_WIDTH,

        // layout / dimension (size/width/height have custom accessors)
        TiC.PROPERTY_WIDTH, TiC.PROPERTY_HEIGHT, TiC.PROPERTY_CENTER,
        TiC.PROPERTY_LEFT, TiC.PROPERTY_TOP, TiC.PROPERTY_RIGHT,
        TiC.PROPERTY_BOTTOM, TiC.PROPERTY_LAYOUT, TiC.PROPERTY_ZINDEX,
        TiC.PROPERTY_HORIZONTAL_WRAP, TiC.PROPERTY_MIN_WIDTH,
        TiC.PROPERTY_MIN_HEIGHT, TiC.PROPERTY_MAX_WIDTH,
        TiC.PROPERTY_MAX_HEIGHT, TiC.PROPERTY_FULLSCREEN, TiC.PROPERTY_WEIGHT,
        TiC.PROPERTY_SIZE_RATIO,

        // accessibility
        TiC.PROPERTY_ACCESSIBILITY_HINT, TiC.PROPERTY_ACCESSIBILITY_LABEL,
        TiC.PROPERTY_ACCESSIBILITY_VALUE, TiC.PROPERTY_ACCESSIBILITY_HIDDEN,

        // others
        TiC.PROPERTY_VISIBLE, TiC.PROPERTY_ENABLED, TiC.PROPERTY_OPACITY,
        TiC.PROPERTY_TOUCH_ENABLED, TiC.PROPERTY_SELECTED,
        TiC.PROPERTY_FOCUSABLE, TiC.PROPERTY_SOFT_KEYBOARD_ON_FOCUS,
        TiC.PROPERTY_TRANSFORM, TiC.PROPERTY_ANCHOR_POINT,
        TiC.PROPERTY_TOUCH_PASSTHROUGH, TiC.PROPERTY_CLIP_CHILDREN,
        TiC.PROPERTY_VIEW_MASK, TiC.PROPERTY_TRANSLATION_Z,
        TiC.PROPERTY_KEEP_SCREEN_ON, "touchTestId", "translationX",
        "translationY", "translationZ", "rotation", "rotationX", "rotationY",
        "scaleX", "scaleY", TiC.PROPERTY_TRANSITION_NAME, TiC.PROPERTY_HIT_RECT,
        TiC.PROPERTY_HIDDEN_BEHAVIOR })
public abstract class TiViewProxy extends AnimatableProxy
    ...
    // private static int defaultTransitionStyle =
    // TransitionHelper.Types.kTransitionSwipe.ordinal();
    // private static int defaultTransitionSubStyle =
    // TransitionHelper.SubTypes.kRightToLeft.ordinal();
    ...
    public boolean handleMessage(Message msg) {
        ...
        case MSG_GETSIZE: {
            ...
            if (view != null) {
                ...
                if (v != null) {
                    TiDimension nativeWidth = new TiDimension(v.getWidth(),
                            TiDimension.TYPE_WIDTH);
                    TiDimension nativeHeight = new TiDimension(v.getHeight(),
                            TiDimension.TYPE_HEIGHT);

                    // TiDimension needs a view to grab the window manager, so
                    // we'll just use the decorview of the current window
                    View decorView = TiApplication.getAppRootOrCurrentActivity()
                            .getWindow().getDecorView();
                    d.put(TiC.PROPERTY_WIDTH,
                            nativeWidth.getAsDefault(decorView));
                    d.put(TiC.PROPERTY_HEIGHT,
                            nativeHeight.getAsDefault(decorView));
                }
            }
            ...
            if (view != null) {
                ...
                if (v != null) {
                    ...
                    if (activity != null) {
                        ...
                        TiApplication.getAppCurrentActivity().getWindowManager()
                        ...
                        if (TiC.KIT_KAT_OR_GREATER) {
                            Window w = activity.getWindow(); // in Activity's
                                                             // onCreate() for
                                                             // instance
                            isTranslucent = (w.getAttributes().flags
                        ...
                        if (!isTranslucent) {
                            ...
                            position[1] -= statusHeight; // we remove statusbar
                                                         // height
                        }

                        // View decorView =
                        // TiApplication.getAppRootOrCurrentActivity().getWindow().getDecorView();
                        TiDimension nativeWidth = new TiDimension(
                                v.getMeasuredWidth(), TiDimension.TYPE_WIDTH);
                        TiDimension nativeHeight = new TiDimension(
                                v.getMeasuredHeight(), TiDimension.TYPE_HEIGHT);
                        d.put(TiC.PROPERTY_WIDTH,
                                nativeWidth.getAsDefault(decorView));
                        d.put(TiC.PROPERTY_HEIGHT,
                                nativeHeight.getAsDefault(decorView));
                        d.put(TiC.PROPERTY_X,
                                new TiDimension(position[0],
                                        TiDimension.TYPE_LEFT)
                                                .getAsDefault(decorView));
                        d.put(TiC.PROPERTY_Y,
                                new TiDimension(position[1],
        ...
        // handleTransitionViews((TiViewProxy)args.get(0),
        // (TiViewProxy)args.get(1), args.get(2));
        // return true;
        // }
        ...
        if (view != null) {
            ...
            if (v != null) {
                ...
                if (activity != null) {
                    ...
                    d = TiConvert.toRectDict(position[0], position[1],
...
//        }
    ...
    public void clearViews() {
        this.view = null;
        // we must use getChildren because of the controls trick in
        // TableViewRowProxy
    ...
    /**
     * Transfer an existing view to this view proxy. Special use in tableView.
     * Do not use anywhere else. Called from TiTableViewRowProxyItem.java
     * 
     * @param transferview
     *            - The view to transfer
     * @param oldProxy
     *            - The currentProxy of the view
     */
    ...
    public TiUIView getOrCreateView(final boolean enableModelListener,
        ...
        return (TiUIView) TiMessenger.sendBlockingMainMessage(getMainHandler()
                .obtainMessage(MSG_GETVIEW, (enableModelListener) ? 1 : 0,
                        (processProperties) ? 1 : 0),
                0);
    }
    ...
    protected TiUIView handleGetView(final boolean enableModelListener,
        ...
        if (view == null) {
            ...
            if (activity == null || activity.isDestroyed()) {
                activity = (TiBaseActivity) TiApplication
                        .getAppRootOrCurrentActivity();
            }
            // Log.d(TAG, "getView: " + getClass().getSimpleName(),
            // Log.DEBUG_MODE);
            ...
            synchronized (children) {
                for (KrollProxy p : children) {
                    if (p instanceof TiViewProxy) {
                        TiUIView cv = ((TiViewProxy) p).getOrCreateView(
                                enableModelListener, processProperties);
                        view.add(cv);
                        if (p instanceof TiWindowProxy
                                && !((TiWindowProxy) p).isOpenedOrOpening()) {
            ...
            if (children != null) {
                synchronized (children) {
                    for (KrollProxy child : children) {
                        if (child instanceof TiViewProxy) {
                            ((TiViewProxy) child)
                                    .releaseViews(activityFinishing);
                        }
                    }
                }
            }
    ...
    /**
     * Implementing classes should use this method to create and return the
     * appropriate view.
     * 
     * @param activity
    ...
    public void handleAdd(TiViewProxy child, int index) {
        if (!(this instanceof DecorViewProxy) && child instanceof TiWindowProxy
                && !((TiWindowProxy) child).isOpenedOrOpening()) {
    ...
    protected void handleShow(KrollDict options) {
        if (view != null) {
            if (TiC.LOLLIPOP_OR_GREATER && TiConvert.toBoolean(options,
                    TiC.PROPERTY_ANIMATED, false)) {
                ...
                android.animation.Animator anim = ViewAnimationUtils
                        .createCircularReveal(nativeView, width / 2, height / 2,
                                0, radius);
                view.show();
                anim.start();
                return;
            ...
            if (!view.blur()) {
                if (children != null) {
                    synchronized (children) {
                        for (KrollProxy child : children) {
                            if (child instanceof TiViewProxy) {
                                if (((TiViewProxy) child).handleBlur())
                                    return true;
                            }
                        }
                    }
    ...
    public TiBlob toImage(
            ...
            @Kroll.argument(optional = true) Object options) {
            ...
            public Bitmap execute() {
                ...
                if (options != null) {
                    if (options instanceof HashMap) {
                        scale = TiConvert.toFloat((HashMap) options,
                                TiC.PROPERTY_SCALE, scale);
                        props = TiConvert.toKrollDict((HashMap) options,
                                TiC.PROPERTY_PROPERTIES, props);
                    } else {
                ...
                Bitmap result = TiUIHelper.viewToBitmap(view.getLayoutParams(),
                        viewForScreenshot());
                if (scale != 1) {
    ...
    @Override
    public void eventListenerAdded(String eventName, int count,
            KrollProxy proxy) {
        super.eventListenerAdded(eventName, count, proxy);
        if (eventName.equals(TiC.EVENT_CLICK) && proxy.equals(this)
                && count == 1 && !(proxy instanceof TiWindowProxy)) {
            if (!proxy.hasProperty(TiC.PROPERTY_TOUCH_ENABLED) || TiConvert
                    .toBoolean(proxy.getProperty(TiC.PROPERTY_TOUCH_ENABLED))) {
                setClickable(true);
            }
        }
    }
    ...
    public void eventListenerRemoved(String eventName, int count,
        ...
        if (eventName.equals(TiC.EVENT_CLICK) && count == 0
                && proxy.equals(this) && !(proxy instanceof TiWindowProxy)) {
            if (proxy.hasProperty(TiC.PROPERTY_TOUCH_ENABLED) && !TiConvert
                    .toBoolean(proxy.getProperty(TiC.PROPERTY_TOUCH_ENABLED))) {
                setClickable(false);
            }
    ...
    // }
    // KrollDict options = TiApplication.getInstance().getStylesheet(baseUrl,
    // classes, null);
    // extend(options);
    // }
    ...
    public KrollDict convertPointToView(Object value, TiViewProxy dest) {
        if (value == null) {
            throw new IllegalArgumentException(
                    "convertPointToView: point must not be null");
        }

        if (dest == null) {
            throw new IllegalArgumentException(
                    "convertPointToView: destinationView must not be null");
        }
        ...
        if (view == null) {
            Log.w(TAG,
                    "convertPointToView: View has not been attached, cannot convert point");
            return null;
        }

        if (destView == null) {
            Log.w(TAG,
                    "convertPointToView: DestinationView has not been attached, cannot convert point");
            return null;
        }
        ...
        // Log.d(TAG, "nativeView location in window, x: " + viewLocation[0] +
        // ", y: " + viewLocation[1], Log.DEBUG_MODE);
        // Log.d(TAG, "destNativeView location in window, x: " + destLocation[0]
        // + ", y: " + destLocation[1], Log.DEBUG_MODE);
        // }
        ...
        float[] points = new float[] { pointWindowX - destLocation[0],
                pointWindowY - destLocation[1] };
     ...
     */
    private boolean checkBorderProps(TiViewProxy oldProxy,
            TiViewProxy newProxy) {
        KrollDict oldProperties = oldProxy.getProperties();
        KrollDict newProperties = newProxy.getProperties();
        boolean oldHasBorder = oldProperties
                .containsKeyAndNotNull(TiC.PROPERTY_BORDER_COLOR)
                || oldProperties
                        .containsKeyAndNotNull(TiC.PROPERTY_BORDER_RADIUS)
                || oldProperties
                        .containsKeyAndNotNull(TiC.PROPERTY_BORDER_WIDTH);
        boolean newHasBorder = newProperties
                .containsKeyAndNotNull(TiC.PROPERTY_BORDER_COLOR)
                || newProperties
                        .containsKeyAndNotNull(TiC.PROPERTY_BORDER_RADIUS)
                || newProperties
    ...
    public Boolean validateTransferToProxy(TiViewProxy newProxy, Boolean deep) {
        ...
        View ourView = ((TiViewProxy) oldProxy.getParent())
                .parentViewForChild(oldProxy);
            ...
            try {
                ...
                for (int i = 0; i < oldproxies.length; i++) {
                    ...
                    if (newSubProxy instanceof TiViewProxy
                        ...
                        TiUIView oldview = ((TiViewProxy) oldSubProxy)
                                .peekView();
                        if (oldview == null) {
                            return false;
                        }
                        if (!((TiViewProxy) oldSubProxy)
                                .validateTransferToProxy(
                                        (TiViewProxy) newSubProxy, true))
                            return false;
                    }
    ...
    private void handleTransitionViews(final TiViewProxy viewOut,
            final TiViewProxy viewIn, HashMap arg,
        ...
        if (viewOut != null && children != null) {
            synchronized (children) {
                index = children.indexOf(viewOut);
                viewOutIsNotChild = index == -1;
            }
        }
        ...
        Transition transition = TransitionHelper
                .transitionFromObject((arg != null) ? arg : null, null, null);
        if (viewToAddTo != null) {
            if (viewIn != null)
                viewIn.setActivity(getActivity());
            final View viewToAdd = (viewIn != null)
                    ? viewIn.getOrCreateView().getOuterView() : null;
            final View viewToHide = (viewOut != null) ? viewOut.getOuterView()
                            : null;
            if (viewToHide !=null) {
                index = viewToAddTo.indexOfChild(viewToHide);
            }
            if (viewToAdd != null) {
                viewToAdd.setVisibility(View.GONE);
                TiUIHelper.addView(viewToAddTo, viewToAdd, index,
                            viewIn.peekView().getLayoutParams()); // make sure it s
                                                                  // removed from
                                                                  // its parent
            }
            ...
            if (transition != null) {
                ...
                AnimatorSet set = transition.getSet(new AnimatorListener() {
                    public void onAnimationEnd(Animator arg0) {
                        ...
                        if (viewOut != null) {
                            if (viewOut != null) {
                                TiCompositeLayout.LayoutParams params = TiCompositeLayout
                                        .getChildParams(viewToHide);
                ...
                Animator additionalAnimator = view.createAnimationLayoutUpdater(
                        viewToAddTo, viewToHide, true);
    ...
    @Kroll.method
    public void transitionViews(final Object viewOut, final Object viewIn,
            @Kroll.argument(optional = true) final HashMap arg,
        ...
        runInUiThread(new CommandNoReturn() {
            @Override
            public void execute() {
                handleTransitionViews((TiViewProxy) viewOut,
                        (TiViewProxy) viewIn, arg, callback);
            }
        }, false);
        // if (TiApplication.isUIThread()) {
        ...
        // args.add(viewIn);
        // args.add(arg);
        // getMainHandler().obtainMessage(MSG_TRANSITION_VIEWS,
        // args).sendToTarget();
        // }
    }
    ...
    @Kroll.method
    public void blurBackground(Object arg,
        ...
        if (TiApplication.isUIThread()) {
            ...
            getMainHandler().obtainMessage(MSG_BLUR_BACKGROUND, options)
                    .sendToTarget();
        }
    }
    ...
    public boolean containsView(TiViewProxy proxy) {
        ...
        for (KrollProxy child : getChildren()) {
            if (child instanceof TiViewProxy) {
                if (((TiViewProxy) child).containsView(proxy))
                    return true;
            }
        }
        return false;
