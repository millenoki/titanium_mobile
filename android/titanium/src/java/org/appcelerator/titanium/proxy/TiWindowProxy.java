...
@SuppressLint("NewApi")
@Kroll.proxy(propertyAccessors = { TiC.PROPERTY_EXIT_ON_CLOSE,
        TiC.PROPERTY_FULLSCREEN, TiC.PROPERTY_ON_BACK, TiC.PROPERTY_TITLE,
        TiC.PROPERTY_TITLEID,
        TiC.PROPERTY_WINDOW_SOFT_INPUT_MODE }, propertyDontEnumAccessors = {
...
public abstract class TiWindowProxy extends TiViewProxy {
    ...
    private TiAnimator animatorFromArgs(HashMap args) {
        if (args == null || args.isEmpty()
                ...
                || TiConvert.toBoolean(args, TiC.PROPERTY_ANIMATED,
                        true) == false) {
            return null;
        }
    ...
    public void open(@Kroll.argument(optional = true) Object arg) {
        if (state == State.OPENED || state == State.OPENING
                || TiApplication.isErrorDialogShowing()) {
            return;
        }
        if (state == State.CLOSING) {
            // the atached activity is going to be destroyed, let's detach
            // ourself from it
            sendCloseEvent(true);
            if (activity != null) {
        ...
        TiMessenger.sendBlockingMainMessage(
                getMainHandler().obtainMessage(MSG_OPEN), options);
    ...
    public void close(@Kroll.argument(optional = true) Object arg) {
        ...
        TiMessenger.sendBlockingMainMessage(
                getMainHandler().obtainMessage(MSG_CLOSE), options);
    }
    ...
    private void sendCloseEvent(boolean activityIsFinishing) {
        ...
        // And it will dispose the handler of the window in the JS if the
        // activity
    ...
    public void closeFromActivity(boolean activityIsFinishing) {
        ...
        if (activityIsFinishing) {
            ...
            // If the activity is forced to destroy by Android OS due to lack of
            // memory or
            // enabling "Don't keep activities" (TIMOB-12939), we will not
            // release the
    ...
    public void addProxyWaitingForActivity(KrollProxy waitingProxy) {
        proxiesWaitingForActivity
                .add(new WeakReference<KrollProxy>(waitingProxy));
    }
    ...
    // {
    // return TiUIHelper.viewToImage(null,
    // getActivity().getWindow().getDecorView(), scale.floatValue());
    // }
        ...
        synchronized (proxiesWaitingForActivity.synchronizedList()) {
            for (KrollProxy proxy : proxiesWaitingForActivity.nonNull()) {
                try {
                    ...
                    Log.e(TAG, "Error attaching activity to proxy: "
                            + t.getMessage(), t);
                }
            }
        }
     ...
     *
     * Default implementation will fire "focus" and "blur" events when the focus
     * state has changed.
     *
     * @param focused
     *            true if focus was gained
     */
    ...
    public void setTitleView(Object data) {
        ...
        if (isOpenedOrOpening()) {
            getActionBar().onPropertyChanged(TiC.PROPERTY_TITLE_VIEW,
                    getHoldedProxy(TiC.PROPERTY_TITLE_VIEW));
        }
    }
    ...
    public void setOrientationModes(int[] modes) {
        ...
        if (modes != null) {
            ...
            for (int i = 0; i < orientationModes.length; i++) {
                ...
                TiDeviceOrientation orientation = TiDeviceOrientation
                        .fromTiIntId(integerId);
                if (orientation != null) {
                    switch (orientation) {
                    case PORTRAIT:
                        hasPortrait = true;
                    ...
                    case LANDSCAPE_LEFT:
                        ...
                        break;
                    default:
                        Log.w(TAG, "'orientationMode' cannot be set to: "
                                + orientation.toTiConstantName());
                        break;
                    }
                } else {
                    Log.w(TAG, "'orientationMode' was given unknown value: "
                            + integerId);
                }
            }

            // determine if we have a valid activity orientation mode based on
            // provided modes list
            if (orientationModes.length == 0) {
                activityOrientationMode = ActivityInfo.SCREEN_ORIENTATION_USER;
                    ...
                    && (hasLandscape || hasLandscapeReverse)) {
                activityOrientationMode = ActivityInfo.SCREEN_ORIENTATION_USER;
            } else if (hasPortrait && hasPortraitReverse) {
                activityOrientationMode = ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT;

                // unable to use constant until sdk lvl 9, use constant value
                // instead
            ...
            // Wait until the window activity is created before setting
            // orientation modes.
            ...
            if (activity != null) {
                if (activity instanceof TiBaseActivity) {
                    activity.setRequestedOrientation(((TiBaseActivity) activity)
                            .getOriginalOrientationMode());
                }
            }
        }
    }
    ...
    @Kroll.getProperty(name = "_internalActivity", enumerable = false)
    ...
    /**
     * Sub-classes will need to call handlePostOpen after their window is
     * visible so any pending dialogs can successfully show after the window is
     * opened
     */
    protected void handlePostOpen() {
        ...
        // Make sure we draw the view during the layout pass. This does not seem
        // to cause another layout pass. We need
    ...
    public boolean shouldExitOnClose() {
        if (hasProperty(TiC.PROPERTY_EXIT_ON_CLOSE))
            return TiConvert.toBoolean(properties, TiC.PROPERTY_EXIT_ON_CLOSE,
                    false);
    ...
    public HashMap getActivityProperties(HashMap properties,
            HashMap actionBarDictSuppl, final boolean canOverWrite,
            final boolean navBarHidden) {
        ...
        if (properties != null) {
            HashMap propsActionBarDict = TiConvert
                    .toHashMap(properties.get(TiC.PROPERTY_ACTION_BAR));
            if (propsActionBarDict != null) {
                if (actionBarDict != null) {
                    actionBarDict = KrollDict.merge(actionBarDict,
                            propsActionBarDict);
                } else {
        ...
        for (String key : ActionBarProxy.windowProps()) {
            if (windowProperties.containsKey(key)) {
                String realKey = TiUtils.mapGetOrDefault(
                        ActionBarProxy.propsToReplace(), key, key);
                if (canOverWrite || actionBarDict == null
                        || !actionBarDict.containsKey(realKey)) {
                    if (actionBarDict == null) {
     ...
     * 
     * @param activity
     *            The activity on which options bundle should be created.
