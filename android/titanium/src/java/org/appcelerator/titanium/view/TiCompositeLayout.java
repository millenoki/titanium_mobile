...
public class TiCompositeLayout extends FreeLayout
    ...
    protected static int getViewWidthPadding(View child, LayoutParams params,
            View parent) {
        int padding = 0;
        padding += Math.abs(getLayoutOptionAsPixelsNoAnim(params.optionLeft,
                TiDimension.TYPE_LEFT, params, parent));
        padding += Math.abs(getLayoutOptionAsPixelsNoAnim(params.optionRight,
                TiDimension.TYPE_RIGHT, params, parent));
        return padding;
    }

    protected static int getViewHeightPadding(View child, LayoutParams params,
            View parent) {
        int padding = 0;
        padding += Math.abs(getLayoutOptionAsPixels(params.optionTop,
                TiDimension.TYPE_TOP, params, parent));
        padding += Math.abs(getLayoutOptionAsPixels(params.optionBottom,
                TiDimension.TYPE_BOTTOM, params, parent));
        return padding;
    }

    private boolean viewShouldFillHorizontalLayout(View view,
            LayoutParams params) {
        if (view.getVisibility() == View.GONE)
            return false;
        if (params.ignoreInFill)
            return false;
        if (params.fullscreen == true)
            return true;
        if (params.sizeOrFillWidthEnabled == false)
            return false;
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int i = 0; i < viewGroup.getChildCount(); i++) {
                ...
                // borderView?params:child.getLayoutParams();
                ViewGroup.LayoutParams childParams = child.getLayoutParams();
                if (childParams instanceof LayoutParams
                        && viewShouldFillHorizontalLayout(child,
    ...
    private boolean viewShouldFillVerticalLayout(View view,
            LayoutParams params) {
        if (view.getVisibility() == View.GONE)
            return false;
        if (params.fullscreen == true)
            return true;
        if (params.sizeOrFillHeightEnabled == false)
            return false;
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int i = 0; i < viewGroup.getChildCount(); i++) {
                ...
                ViewGroup.LayoutParams childParams = child.getLayoutParams();
                if (childParams instanceof LayoutParams
                        && viewShouldFillVerticalLayout(child,
                                (LayoutParams) childParams)) {
                    return true;
                }
            }
    ...
    public static TiCompositeLayout.LayoutParams getChildParams(View child) {
        ...
        if (child.getLayoutParams() instanceof TiCompositeLayout.LayoutParams) {
            params = (TiCompositeLayout.LayoutParams) child.getLayoutParams();
        } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 0) {
            ...
            for (int i = 0; i < childCount; i++) {
                // Apply a width and height to the next child view owned by this
                // layout.
                ...
                // if (params.notTiLayout) {
                //// int widthPadding = getViewWidthPadding(child, params,
                // this);
                //// int heightPadding = getViewHeightPadding(child, params,
                // this);
                ...
                // child.getMeasuredHeight() + child.getPaddingTop() +
                ...
                if (horizontalNoWrap
                        && viewShouldFillHorizontalLayout(child, params)) {
                    mAutoFillWidthViews.add(child);
                    autoFillWidthTotalWeight += params.weight;
//                    final android.view.ViewGroup.LayoutParams ourLayoutParams = getTiLayoutParams();
                    needsProcessing =  false;
                }
                if ((vertical || horizontalWrap)
                        && viewShouldFillVerticalLayout(child, params)) {
...
//                    final android.view.ViewGroup.LayoutParams ourLayoutParams = getTiLayoutParams();
                ...
                constrainChild(child, params,
                        horizontalNoWrap ? (w - horizontalRowWidth) : w, wMode,
                        h, hMode, widthPadding, heightPadding);
                ...
                if (horizontal) {
                    if (enableHorizontalWrap) {
                        ...
                        // For horizontal layout without wrap, just keep on
                        // adding
                        // the widths since it doesn't wrap
                        maxWidth += childWidth;
                    }
                    horizontalRowHeight = Math.max(horizontalRowHeight,
                            childHeight);
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    final int childW = (int) (Math.max(0, w - maxWidth) * weight
                            / (autoFillWidthTotalWeight - counter));
                    counter += weight;

                    final int widthPadding = getViewWidthPadding(child, params,
                            this);
                    final int heightPadding = getViewHeightPadding(child,
                            params, this);
                    constrainChild(child, params, childW, wMode, h, hMode,
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    TiCompositeLayout.LayoutParams params = (TiCompositeLayout.LayoutParams) child
                            .getLayoutParams();
                    final float weight = params.weight;
                    final int childH = (int) (Math.max(0, h - maxHeight)
                            * weight / (autoFillHeightTotalWeight - counter));
                    counter += weight;
                    ...
                    final int heightPadding = getViewHeightPadding(child,
                            params, this);
                    constrainChild(child, params, w, wMode, childH, hMode,
                            widthPadding, heightPadding);
                    final int childWidth = child.getMeasuredWidth()
                            + widthPadding;
                    final int childHeight = child.getMeasuredHeight()
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    int heightPadding = getViewHeightPadding(child, params,
                            this);
                    int childwMode = getChildMeasureSpec(maxWidth,
                            MeasureSpec.UNSPECIFIED, widthPadding,
                            params.width);
                    int childhMode = getChildMeasureSpec(maxHeight,
                            MeasureSpec.UNSPECIFIED, heightPadding,
                            params.height);
        ...
        // if we are in a borderView our layoutParams are not the ones we wnat
        // to look at
        ViewGroup.LayoutParams viewParams = getTiLayoutParams();
        LayoutParams p = (viewParams instanceof LayoutParams)
                ? (LayoutParams) viewParams : null;
        if (p != null) {
    ...
    protected void applyAnimationSize(final View child, final LayoutParams p,
            final boolean shouldMeasure) {
        ...
        if (p instanceof AnimationLayoutParams) {
            ...
            if (fraction < 1.0f) {
                ...
                if (startRect != null) {
                    ...
                    measuredWidth = (int) Math.ceil(measuredWidth * fraction
                            + (1 - fraction) * startRect.width());
                    int wMode = MeasureSpec.makeMeasureSpec(measuredWidth,
                            MeasureSpec.EXACTLY);

                    measuredHeight = (int) Math.ceil(measuredHeight * fraction
                            + (1 - fraction) * startRect.height());
                    int hMode = MeasureSpec.makeMeasureSpec(measuredHeight,
                            MeasureSpec.EXACTLY);
                    if (!shouldMeasure && child instanceof TiCompositeLayout) {
                        ((TiCompositeLayout) child).setMeasuredDimension(
    ...
    protected void measureChild(final View child, final LayoutParams p,
            int wMode, int hMode, int widthPadding, int heightPadding,
    ...
    public static int getChildMeasureSpec(int size, int mode, int padding,
            int childDimension) {
    ...
    protected void constrainChild(View child, View parent, LayoutParams p,
            int width, int wMode, int height, int hMode, int widthPadding,
            int heightPadding) {
        if (!p.fullscreen && !p.notTiLayout) {
            int sizeFillConflicts[] = { NOT_SET, NOT_SET };
            boolean checkedForConflict = false;

            // If autoFillsWidth is false, and optionWidth is null, then we use
            // size
            // behavior.
            ...
            wMode = getChildMeasureSpec(width, wMode, widthPadding,
                    childDimension);

            // If autoFillsHeight is false, and optionHeight is null, then we
            // use
            // size behavior.
            ...
            if (p.optionHeight != null) {
                ...
                // again
                if (p.autoFillsHeight || (checkedForConflict
                        && sizeFillConflicts[1] == HAS_SIZE_FILL_CONFLICT)) {
                    childDimension = LayoutParams.MATCH_PARENT;
            ...
            hMode = getChildMeasureSpec(height, hMode, heightPadding,
                    childDimension);
        }
        measureChild(child, p, wMode, hMode, widthPadding, heightPadding, width,
                height);
    }

    protected void constrainChild(View child, LayoutParams p, int width,
            int wMode, int height, int hMode, int widthPadding,
            int heightPadding) {
        constrainChild(child, this, p, width, wMode, height, hMode,
                widthPadding, heightPadding);
    }

    public void constrainChild(View child, View parent, int widthMeasureSpec,
        ...
        int wMode = MeasureSpec.getMode(widthMeasureSpec);
        int hMode = MeasureSpec.getMode(heightMeasureSpec);
        constrainChild(child, parent, params, wFromSpec, wMode, hFromSpec,
                hMode, widthPadding, heightPadding);
    ...
    // int parentRight, int parentWidth, int measuredWidth, boolean
    // canResizeFill) {
    ...
    // if (canResizeFill && params.sizeOrFillWidthEnabled &&
    // params.autoFillsWidth) {
    // return parentWidth - getLayoutOptionAsPixels(params.optionRight,
    // TiDimension.TYPE_RIGHT, params, this) -
    // getLayoutOptionAsPixels(params.optionLeft, TiDimension.TYPE_LEFT ,
    // params, this);
    // }
    // else
    // return width;
    ...
    // if (left != null) {
    // if (centerX != null) {
    // width = (getLayoutOptionAsPixels(centerX, TiDimension.TYPE_CENTER_X,
    // params, this) -
    // getLayoutOptionAsPixels(left, TiDimension.TYPE_LEFT, params, this) -
    // parentLeft) * 2;
    // } else if (right != null) {
    // width = parentWidth - getLayoutOptionAsPixels(right,
    // TiDimension.TYPE_RIGHT, params, this) -
    // getLayoutOptionAsPixels(left, TiDimension.TYPE_LEFT, params, this);
    // }
    // } else if (centerX != null && right != null) {
    // width = (parentRight - getLayoutOptionAsPixels(right,
    // TiDimension.TYPE_RIGHT, params, this) -
    // getLayoutOptionAsPixels(centerX, TiDimension.TYPE_CENTER_X , params,
    // this)) * 2;
    // }
    // return width;
    // }
    ...
    // int parentBottom, int parentHeight, int measuredHeight, boolean
    // canResizeFill) {
    // if (params.fullscreen) {
    ...
    // if (canResizeFill && params.sizeOrFillHeightEnabled &&
    // params.autoFillsHeight) {
    // return parentHeight - getLayoutOptionAsPixels(params.optionTop,
    // TiDimension.TYPE_TOP, params, this) -
    // getLayoutOptionAsPixels(params.optionBottom, TiDimension.TYPE_BOTTOM ,
    // params, this);
    // }
    // else
    // return height;
    ...
    // if (top != null) {
    // if (centerY != null) {
    // height = (getLayoutOptionAsPixels(centerY, TiDimension.TYPE_CENTER_Y,
    // params, this) -
    // getLayoutOptionAsPixels(top, TiDimension.TYPE_TOP, params, this) -
    // parentTop) * 2;
    // } else if (bottom != null) {
    // height = parentHeight - getLayoutOptionAsPixels(top,
    // TiDimension.TYPE_TOP, params, this) -
    // getLayoutOptionAsPixels(bottom, TiDimension.TYPE_BOTTOM, params, this);
    // }
    // } else if (centerY != null && bottom != null) {
    // height = (parentBottom - getLayoutOptionAsPixels(bottom,
    // TiDimension.TYPE_BOTTOM, params, this) -
    // getLayoutOptionAsPixels(centerY, TiDimension.TYPE_CENTER_Y , params,
    // this)) * 2;
    // }
    //
    // return height;
    ...
    protected static int getMeasuredHeightStatic(int maxHeight,
            int heightSpec) {
    ...
    public int getChildSize(final View child, final int index,
            final TiCompositeLayout.LayoutParams params, final int left,
            final int top, final int bottom, final int right, int currentHeight,
            final int[] horizontal, final int[] vertical,
            final boolean firstVisibleChild) {
        ...
        if (horizontalArr) {
            ...
            computePosition(this, params, params.optionLeft,
                    params.optionCenterX, params.optionWidth,
            ...
            if (verticalArr) {
                ...
                computePosition(this, params, params.optionTop,
                        params.optionCenterY, params.optionHeight,
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (needsSort) {
            ...
            if (count > 1) { // No need to sort one item.
                for (int i = 0; i < count; i++) {
                    View child = getChildAt(i);
                    TiCompositeLayout.LayoutParams params = getChildParams(
                            child);
                    params.index = i;
        ...
        for (int i = 0; i < count; i++) {
            ...
            // }
            child.layout(horizontal[0], vertical[0], horizontal[1],
                    vertical[1]);
            currentHeight += vertical[1] - vertical[0];

            currentHeight += getLayoutOptionAsPixels(params.optionTop,
                    TiDimension.TYPE_TOP, params, this);
            currentHeight += getLayoutOptionAsPixels(params.optionBottom,
                    TiDimension.TYPE_BOTTOM, params, this);
        }
    ...
    public static void computePosition(final View parent,
            final LayoutParams params, final TiDimension leftOrTop,
            final TiDimension optionCenter,
            final TiDimension optionWidthOrHeight,
            final TiDimension rightOrBottom, final int measuredSize,
            final int layoutPosition0, final int layoutPosition1, int[] pos,
            final boolean vertical) {
        int dist = layoutPosition1 - layoutPosition0;
        final boolean leftTopDef = (leftOrTop != null
                && !leftOrTop.isUnitUndefined());
        final boolean rightBotDef = (rightOrBottom != null
                && !rightOrBottom.isUnitUndefined());
        final boolean widthHeightDef = (optionWidthOrHeight != null
                && !optionWidthOrHeight.isUnitUndefined());
        if (optionCenter != null && !optionCenter.isUnitUndefined()
            ...
            pos[0] = layoutPosition0
                    + getLayoutOptionAsPixelsNoAnim(optionCenter,
                            optionCenter.getValueType(), params, parent)
                    - halfSize;
            pos[1] = pos[0] + measuredSize;
        } else if ((!leftTopDef && !rightBotDef)
                || (leftTopDef && rightBotDef && widthHeightDef)) {
            // Center
            int offset = (dist - measuredSize) / 2;

            pos[0] = layoutPosition0 + offset;
            // we need to update horizontal and vertical with animationFraction
            // because computePosition
            // will assume 0 for optionLeft==null when it should be
            // startRect.left
            if (params instanceof AnimationLayoutParams
                ...
                if (animFraction < 1.0f) {
                    ...
                    if (startRect != null) {
                        pos[0] = (int) Math.floor(pos[0] * animFraction
                                + (1 - animFraction) * (vertical ? startRect.top
                                        : startRect.left));
                    }
                    // if (startRect != null ) {
                    // if (animP.optionWidth != null) {
                    // toUseWidth = (int) Math.floor(animP.finalWidth *
                    // animP.animationFraction + (1 - animP.animationFraction)*
                    // toUseWidth);
                    // }
                    // if (animP.optionHeight != null) {
                    // toUseHeight = (int) Math.floor(animP.finalHeight *
                    // animP.animationFraction + (1 - animP.animationFraction)*
                    // toUseHeight);
                    // }
                    // }
            ...
            int leftOrTopPixels = getLayoutOptionAsPixels(leftOrTop,
                    leftOrTop.getValueType(), params, parent);
            int rightOrBottomPixels = getLayoutOptionAsPixels(rightOrBottom,
                    rightOrBottom.getValueType(), params, parent);
    ...
    private void computeVerticalLayoutPosition(final int currentHeight,
            final TiDimension optionTop, final int measuredHeight,
            final int layoutTop, int[] pos, final int maxBottom,
            final LayoutParams params) {
        int top = layoutTop + currentHeight;
        top += (optionTop != null) ? getLayoutOptionAsPixels(optionTop,
                TiDimension.TYPE_TOP, params, this) : 0;
        pos[0] = top;
        pos[1] = top + measuredHeight;
    }

    private static int getLayoutOptionAsPixelsNoAnim(final TiDimension option,
            final int type, final LayoutParams params, View parent) {
        return (!params.fullscreen && option != null)
                ? option.getAsPixels(parent) : 0;
    }

    private static int getLayoutOptionAsPixels(final TiDimension option,
            final int type, final LayoutParams params, final View parent) {
        return getLayoutOptionAsPixels(option, type, params,
                parent.getMeasuredWidth(), parent.getMeasuredHeight());
    }

    private static int getLayoutOptionAsPixels(final TiDimension option,
            final int type, final LayoutParams params, final int parentWidth,
            final int parentHeight) {
        // never called with width or height so we can set 0 in fullscreen
        int result = (!params.fullscreen && option != null)
                ? option.getAsPixels(parentWidth, parentHeight) : 0;
        if (params instanceof AnimationLayoutParams) {
            AnimationLayoutParams animP = (AnimationLayoutParams) params;
            float fraction = animP.animationFraction;
            ...
            if (fraction < 1.0f) {
                ...
                case TiDimension.TYPE_LEFT:
                    ...
                    if (option != null && oldParam == null
                            && oldTiParams.optionRight == null) {
                ...
                result = (int) Math
                        .floor(result * fraction + (1 - fraction) * oldValue);
            }
        }
        return result;
    }
    ...
    private void computeHorizontalLayoutPosition(
        ...
        int optionLeftValue = getLayoutOptionAsPixels(optionLeft,
                TiDimension.TYPE_LEFT, params, this);
        left += optionLeftValue;
        horiztonalLayoutPreviousRight = getLayoutOptionAsPixels(optionRight,
                TiDimension.TYPE_RIGHT, params, this);
        ...
        if (enableHorizontalWrap
                && ((right + horiztonalLayoutPreviousRight) > layoutRight
                        || left >= layoutRight)) {
        ...
        computePosition(this, params, params.optionTop, params.optionCenterY,
                params.optionHeight, params.optionBottom, measuredHeight,
                layoutTop, layoutBottom, vpos, true);
        if (!params.autoFillHeight() && (params.optionTop != null
                && !params.optionTop.isUnitUndefined()
                && params.optionBottom != null
                && !params.optionBottom.isUnitUndefined())) {
    ...
    private static boolean hasSizeFillConflict(View parent, int[] conflicts,
        ...
        if (parent instanceof TiCompositeLayout) {
            ...
            // behavior.
            if (firstIteration && (currentParams.autoFillsWidth
                    || currentParams.optionWidth != null)) {
                conflicts[0] = NO_SIZE_FILL_CONFLICT;
            }
            if (firstIteration && (currentParams.autoFillsHeight
                    || currentParams.optionHeight != null)) {
    ...
    public static class LayoutParams extends FreeLayout.LayoutParams {
        ...
        private void initiateFromOther(
                final TiCompositeLayout.LayoutParams params) {
        ...
        public void setWidth(int width) {
            this.width = width;
            sizeOrFillWidthEnabled = (width == MATCH_PARENT
                    || width == WRAP_CONTENT);
        ...
        public void setHeight(int height) {
            this.height = height;
            sizeOrFillHeightEnabled = (height == MATCH_PARENT
                    || height == WRAP_CONTENT);
        ...
        public boolean percentWidth() {
            return (this.optionWidth != null
                    && this.optionWidth.isUnitPercent());
        }

        public boolean fixedSizeHeight() {
            return (this.optionHeight != null
                    && this.optionHeight.isUnitFixed());
        }
        public boolean percentHeight() {
            return (this.optionHeight != null
                    && this.optionHeight.isUnitPercent());
        }
    }
    ...
    public static boolean viewContainsTouch(final View view, final double rawx,
            final double rawy, int[] location) {
        if (location == null) {
            location = new int[2];
        }
    ...
    public boolean canScroll() {
        ...
        return !mInterTouchPassThrough || (view != null && view.isTouchEnabled
                && !view.touchPassThrough);
    }
            ...
            if (mInterTouchPassThrough == true
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (viewContainsTouch(child, x, y, location)) {
                        tag = child.getTag();
                        if (tag == null
                                || (tag instanceof TiUIView && !((TiUIView) tag)
                                        .touchPassThrough(child, event))) {
                            return false;
                        } else {
                            return false;
                        }
