...
public class TiLoadImageManager implements Handler.Callback
{
	...
	public void loadSync(TiDrawableReference imageref, TiLoadImageListener listener)
	{
		try {
			...
			listener.loadImageFinished(imageref, drawable);
		} catch (Exception e) {
			listener.loadImageFailed();
		}
	}

	protected void handleLoadImageMessage(int what, TiDrawableReference imageref, Drawable drawable)
	{
		ArrayList<SoftReference<TiLoadImageListener>> toRemove = new ArrayList<SoftReference<TiLoadImageListener>>();
		synchronized (listeners) {
			ArrayList<SoftReference<TiLoadImageListener>> listenerList = listeners.get(imageref.hashCode());
			for (SoftReference<TiLoadImageListener> listener : listenerList) {
				...
				if (l != null) {
					if (what == MSG_FIRE_LOAD_FINISHED) {
						l.loadImageFinished(imageref, drawable);
	...
	public boolean handleMessage(Message msg)
	{
		...
		switch (msg.what) {
			case MSG_FIRE_LOAD_FINISHED:
				handleLoadImageMessage(MSG_FIRE_LOAD_FINISHED, job.imageref, job.resultDrawable);
				return true;
			case MSG_FIRE_LOAD_FAILED:
				if ((Integer)msg.arg2 == 1) { //retry download
					...
					try {
						...
						handleLoadImageMessage(MSG_FIRE_LOAD_FAILED, job.imageref, null);
					}
				}
				else {
					handleLoadImageMessage(MSG_FIRE_LOAD_FAILED, job.imageref, null);
	...
	protected class LoadImageJob implements Runnable
	{
        ...
        protected Drawable resultDrawable;
		...
		{
			try {
				this.resultDrawable = imageref.getDrawable(true);
				synchronized (loadingImageRefs) {
				...
				if (resultDrawable instanceof BitmapDrawable) {
					Bitmap b = ((BitmapDrawable) resultDrawable).getBitmap();
					if (b == null) {
						Message msg = handler.obtainMessage(MSG_FIRE_LOAD_FAILED);
						msg.obj = this;
						msg.arg1 = imageref.hashCode();
					...
					if (b.getWidth() > 4096 || b.getHeight() > 4096) { //too big!
						...
						try {
						    this.resultDrawable = new BitmapDrawable(TiApplication.getInstance().getResources(), Bitmap.createScaledBitmap(b, dstWidth, dstHeight, true));
