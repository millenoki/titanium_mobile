...
class TiHtmlToSpannedConverter implements ContentHandler {
    ...
    public TiHtmlToSpannedConverter(Html.ImageGetter imageGetter,
        ...
        if (style != null) {
            ...
            for (String item : items) {
                ...
                if (key.equals("background-color")) {
                    ...
                    strokeWidth = TiConvert.toTiDimension(value,
                            TiDimension.TYPE_HEIGHT).getAsPixels();
                    needsBgdSpan = true;
                } else if (key.equals("border-radius")) {
                    radius = TiConvert.toTiDimension(value,
                            TiDimension.TYPE_HEIGHT).getAsPixels();
                    needsBgdSpan = true;
                }
            }
            if (needsBgdSpan)
                return new CustomBackgroundSpan(radius, fillColor, strokeColor,
                        strokeWidth);
        }
        // if (attr.get(key))
        return null;
