...
public class ColorArt {
    ...
    private final float MINIMUM_CONTRAST_THRESHOLD = 1.4f;
    ...
    private Integer mRandomThreshold = -1;
    ...
    public ColorArt(Bitmap bitmap, int scaleWidth, int scaleHeight) {
        mBitmap = Bitmap.createScaledBitmap(bitmap, scaleWidth, scaleHeight, false);
//        mBitmap = bitmap;
        analyzeImage();
    }

    private void analyzeImage() {
        mRandomThreshold = (int) (mBitmap.getHeight() * COLOR_THRESHOLD_MINIMUM_PERCENTAGE);
...
//                Log.d(LOG_TAG, TiColorHelper.toHexString(pixel));
                if (y == 0 || x == 0) {
                    leftImageColors.add(pixel);
                }
                mImageColors.add(pixel);
            }
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (colorCount <= mRandomThreshold) {
                continue;
            }
...
//            Log.d(LOG_TAG, TiColorHelper.toHexString(currentContainer.getColor()) + ", " + currentContainer.getCount());
            if (mPrimaryColor == null) {
