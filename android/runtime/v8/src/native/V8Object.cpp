	...
	(JNIEnv *env, jobject object, jlong ptr, jstring name)
{
	...
	Local<Value> jsValue = Local<Value>::New(V8Runtime::v8_isolate, jsObject->Get(jsName));
	bool isNew;
	return TypeConverter::jsValueToJavaObject(V8Runtime::v8_isolate, env, jsValue, &isNew);
}
	...
	(JNIEnv *env, jobject object, jlong ptr, jobject jprops)
{
	...
	Local<Object> proxyProperties = jsObject->Get(titanium::Proxy::propertiesSymbol.Get(V8Runtime::v8_isolate)).As<Object>();
	Handle<Object> properties = TypeConverter::javaHashMapToJsValue(V8Runtime::v8_isolate, env, jprops);
	...
	for (int i = 0; i < length; ++i) {
		...
		if (name->IsString()) {
			...
			if (!jsObject->HasRealNamedCallbackProperty(nameString)
				&& !jsObject->HasRealNamedProperty(nameString)) {
				jsObject->Set(name, value);
				isProperty = false;
			}
		}
		if (isProperty) {
	...
	(JNIEnv *env, jobject jEmitter, jlong ptr, jobject jsource, jlong sourcePtr, jstring event, jobject data, jboolean bubble, jboolean reportSuccess, jint code, jstring errorMessage)
{
	...
	Local<Context> context = V8Runtime::v8_isolate->GetCurrentContext();
	MaybeLocal<Value> fireEventValue = emitter->Get(context, EventEmitter::emitSymbol.Get(V8Runtime::v8_isolate));
