...
if [ "$ANDROID_PLATFORM" = "" ]; then
	...
	if [ "$ANDROID_SDK" = "" ]; then
		...
		if [ "$ANDROID_SDK" = "" ]; then
			echo "Error: The path to the Android SDK platform must be set in the ANDROID_PLATFORM environment variable (e.g. /opt/android-sdk-macosx/platforms/android-21)"
			exit 1
		fi
	fi
	export ANDROID_PLATFORM="$(cd "$ANDROID_SDK"; pwd)/platforms/android-21"
fi
