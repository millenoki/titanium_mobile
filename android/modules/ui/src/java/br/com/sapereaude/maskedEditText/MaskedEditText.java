...
public class MaskedEditText extends EditText implements TextWatcher {
    ...
    private boolean isTextEmptyOrNull() {
        return (rawText.length() == 0 && (hasHint() || getText() == null || getText().length() == 0));
    }
    ...
    private String makeMaskedText() {
        char[] maskedText = mask.toCharArray();
        for(int i = 0; i < rawToMask.length; i++) {
            if(i < rawText.length()) {
                maskedText[rawToMask[i]] = rawText.charAt(i);
            }
            else {
                maskedText[rawToMask[i]] = charRepresentation;
            }
        }
