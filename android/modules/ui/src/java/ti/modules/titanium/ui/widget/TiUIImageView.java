...
public class TiUIImageView extends TiUINonViewGroupView
        implements OnLifecycleEvent, Handler.Callback, TiDrawableTarget {
    ...
    protected static final int TIFLAG_NEEDS_SCALE_TYPE               = 0x00000001;
    ...
    private class FilterAndSetTask extends AsyncTask<Bitmap, Void, Bitmap> {
        ...
        FilterAndSetTask(final TiDrawableReference imageRef,
        ...
        protected Bitmap doInBackground(Bitmap... params) {
            ...
            Pair<Bitmap, KrollDict> result = TiImageHelper.imageFiltered(bitmap,
                    filterOptions, imageRef.getCacheKey(), false);
            if (result != null) {
    ...
    static {
        ...
        tmp.add(TiC.PROPERTY_LOCAL_LOAD_SYNC);
        tmp.add(TiC.PROPERTY_ONLY_TRANSITION_IF_REMOTE);
        tmp.add(TiC.PROPERTY_SCALE_TYPE);
        tmp.add(TiC.PROPERTY_TRANSITION);
        KEY_SEQUENCE = tmp;
    }
    ...
    public TiUIImageView(final TiViewProxy proxy) {
        ...
        setImage(null, false); // this actually creates a drawable which will
                               // allow
        // transition
    ...
    private void setImage(final Bitmap bitmap, boolean shouldTransition) {
        setDrawable(
                (bitmap != null) ? new BitmapDrawable(
                        getContext().getResources(), bitmap) : null,
                shouldTransition);
    }
    ...
    private void setDrawable(final Drawable drawable,
        ...
        if (!TiApplication.isUIThread()) {
            mainHandler.obtainMessage(SET_DRAWABLE, shouldTransition ? 1 : 0, 0,
                    drawable).sendToTarget();
        } else {
    ...
    private void handleSetDrawable(final Drawable drawable,
        ...
        TiImageView view = getView();
        if (view != null) {
            view.setImageDrawableWithTransition(drawable,
    ...
    private void setImages() {
        ...
        if (loader == null) {
            ...
            Log.d(TAG,
                    "STARTING LOADER THREAD " + loaderThread + " for " + this,
                    Log.DEBUG_MODE);
            loaderThread.start();
        }
        ...
        for (int i = 0; i < animatedImageSources.size(); i++) {
            ...
            if (!imageref.isNetworkUrl()) {
                ...
                if (drawable != null) {
                    ...
                    Log.e(TAG, "Could not find image for url "
                            + imageref.getUrl());
                }
            }
        }
    ...
    private class Animator extends TimerTask {
        ...
        public void run() {
            ...
            try {
                ...
                ArrayBlockingQueue<BitmapWithIndex> bitmapQueue = loader
                        .getBitmapQueue();
    ...
    public int getCurrentIndex() {
        int total = (animDrawable != null) ? animDrawable.getNumberOfFrames()
                : 1;
        int result = (animDrawable != null) ? animDrawable.getCurrentFrame()
                : currentIndex;
    ...
    public float getProgress() {
        int total = (animDrawable != null) ? animDrawable.getNumberOfFrames()
                : 1;
        float result = ((animDrawable != null) ? animDrawable.getCurrentFrame()
                : currentIndex) / (float) total;
    ...
    public void setCurrentIndex(int index) {
        if (!TiApplication.isUIThread()) {
            TiMessenger.sendBlockingMainMessage(
                    mainHandler.obtainMessage(SET_INDEX),
    ...
    public void setProgress(float progress) {
        if (!TiApplication.isUIThread()) {
            TiMessenger.sendBlockingMainMessage(
                    mainHandler.obtainMessage(SET_PROGRESS),
    ...
    public void start() {
        currentIndex = (animDrawable != null)
                ? (reverse ? animDrawable.getNumberOfFrames() - 1 : 0) : 0;
        if (!TiApplication.isUIThread()) {
    ...
    private void setImageSource(Object object) {
        ...
        if (object instanceof Object[]) {
            ...
            imageSources
                    .add(TiDrawableReference.fromObject(getProxy(), object));
        }
    }
    ...
    private void setDefaultImage(final boolean withTransition) {
        ...
        if (view == null
        // || (!withTransition && proxy.viewInitialised() && transitionDict !=
        // null)
        ) {
            return;
        }
    ...
    protected void didProcessProperties() {
        ...
        if ((mProcessUpdateFlags & TIFLAG_NEEDS_LAYOUT) != 0) {
            if (view != null) {
                ...
                if ((mProcessUpdateFlags & TIFLAG_NEEDS_SCALE_TYPE) != 0) {
                    view.updateScaleType();
                    mProcessUpdateFlags &= ~TIFLAG_NEEDS_SCALE_TYPE;
                }
                // If height and width is not defined, disable scaling for
                // scrollview
                // since an image
                // can extend beyond the screensize in scrollview.
                if (proxy.getParent() instanceof ScrollViewProxy
                        && !view.getHeightDefined()
    ...
    public void didRealize() {
        super.didRealize();
        boolean animating = TiConvert.toBoolean(proxy.getProperty("animating"),
                false);
        if (animating) {
            start();
        }
...
//             view.setAnimationDuration( TiConvert.toInt(newValue));
        ...
        case TiC.PROPERTY_FILTER_OPTIONS:
            ...
            if (currentImage != null
                    && currentImage instanceof BitmapDrawable) {
        ...
        if (view != null) {
            ...
            if (drawable != null) {
                ...
                return bitmap == null ? null
                        : TiBlob.blobFromObject(bitmap, null,
    ...
    public void release() {
        ...
        if (currentDrawable instanceof BitmapDrawable) {
            ...
            TiBitmapPool.decrementRefCount(
                    ((TiNinePatchDrawable) currentDrawable).getBitmap());
        }
        handleStop();
        super.release();
    ...
    public void onBitmapLoaded(Bitmap bitmap, LoadedFrom from) {
        ...
        boolean transition = !onlyTransitionIfRemote
