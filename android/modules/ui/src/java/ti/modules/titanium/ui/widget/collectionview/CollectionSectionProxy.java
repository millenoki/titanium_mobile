...
public class CollectionSectionProxy extends AbsListSectionProxy {
    ...
    public void propertySet(String key, Object newValue, Object oldValue,
        ...
        case TiC.PROPERTY_HEADER_TITLE: {
            ...
            if (newValue instanceof String) {
                addProxyToHold(
                        TiAbsListView
                                .headerViewDict(TiConvert.toString(newValue)),
        ...
        case TiC.PROPERTY_FOOTER_TITLE: {
            ...
            if (newValue instanceof String) {
                addProxyToHold(
                        TiAbsListView
                                .footerViewDict(TiConvert.toString(newValue)),
    ...
    // public int getUserItemInversedIndexFromSectionPosition(final int
    // position) {
    // int result = position;
    // if (hasHeader()) {
    // result -= 1;
    ...
    public HashMap getListItem(int itemPosition) {
        ...
        if (hasHeader && itemPosition == RecyclerView.NO_POSITION) {
            ...
            if (item instanceof HashMap
                    && getHoldedProxy(TiC.PROPERTY_HEADER_VIEW) == null) {
                return (HashMap) item;
            }
            return null;
        }
        ...
        if (hasFooter() && itemPosition == getItemCount()) {
            ...
            if (item instanceof HashMap
                    && getHoldedProxy(TiC.PROPERTY_FOOTER_VIEW) == null) {
                return (HashMap) item;
            }
            return null;
        }
    ...
    public String getTemplateByIndex(int itemPosition) {
        ...
        if (hasFooter() && itemPosition == getItemCount() - 1) {
    ...
    public void setVisible(boolean value) {
        ...
        if (adapter instanceof TiBaseAdapter) {
            if (hidden) {
                ((TiBaseAdapter) adapter).collapseSection(this.sectionIndex,
                        true);
            } else {
                ((TiBaseAdapter) adapter).expandSection(this.sectionIndex,
                        true);
            }
        } else {
            notifyDataChange();
