            ...
            if (newValue != null) {
                if (thumbDrawable == null) {
                    seekBar.getThumb().setColorFilter(
                            TiConvert.toColor(newValue), Mode.SRC_IN);
                }
...
//        proxy.setProperty("maxRange", maxRange);
    ...
    private void updateThumb(String thumbImage) {
        ...
        if (thumbImage != null) {
            ...
            if (thumb != null) {
                ...
                Log.e(TAG, "Unable to locate thumb image for progress bar: "
                        + url);
            }
        } else {
    ...
    private void updateTrackingImages() {
        ...
        if (rightDrawable == null && leftDrawable == null) {
            ...
            Drawable[] lda = { new ClipDrawable(leftDrawable, Gravity.LEFT,
                    ClipDrawable.HORIZONTAL) };
            ld = new LayerDrawable(lda);
            ld.setId(0, android.R.id.progress);
        } else if (leftDrawable == null) {
            rightClipDrawable = new ClipDrawable(rightDrawable, Gravity.RIGHT,
                    ClipDrawable.HORIZONTAL);
            Drawable[] lda = { rightClipDrawable };
            ld = new LayerDrawable(lda);
            ld.setId(0, android.R.id.secondaryProgress);
        } else {
            Drawable[] lda = { rightDrawable, new ClipDrawable(leftDrawable,
    ...
    public void onProgressChanged(SeekBar seekBar, int progress,
        ...
        Log.d(TAG,
                "Progress " + seekBar.getProgress() + " ScaleFactor "
                        + scaleFactor + " Calculated Position " + pos
                        + " ScaledValue " + scaledValue + " Min " + min + " Max"
                        + max + " MinRange" + minRange + " MaxRange" + maxRange,
                Log.DEBUG_MODE);
        if (hasListeners(TiC.EVENT_CHANGE, false)) {
            Drawable thumb = (thumbDrawable != null) ? thumbDrawable.get()
                    : null;
            ...
            if (thumb != null) {
                ...
                if (thumbBounds != null) {
                    offset.put(TiC.EVENT_PROPERTY_X,
                            thumbBounds.left - seekBar.getThumbOffset());
