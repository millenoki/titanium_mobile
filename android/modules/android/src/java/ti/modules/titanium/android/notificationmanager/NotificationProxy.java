...
public class NotificationProxy extends ReusableProxy
    ...
    public static NotificationProxy fromObject(Object obj) {
        if (obj instanceof NotificationProxy) {
            ...
            NotificationProxy proxy = (NotificationProxy) createProxy(
                    NotificationProxy.class, null, new Object[] { obj }, null);
            return proxy;
        }
        return null;
    }
    ...
    public NotificationProxy() {
        super();
        final String channelId = NotificationManagerModule.useDefaultChannel()
                ? NotificationManagerModule.DEFAULT_CHANNEL_ID
                : "miscellaneous";
        notificationBuilder = new NotificationCompat.Builder(
                TiApplication.getAppContext(), channelId)
    ...
    protected void didProcessProperties() {
        ...
        if ((mProcessUpdateFlags & TIFLAG_NEEDS_UPDATE) != 0) {
            if (this.currentId >= 0) {
                ...
                Log.d(TAG, "updating notification " + this.currentId,
                        Log.DEBUG_MODE);
    ...
    public void propertySet(String key, Object newValue, Object oldValue,
        ...
        case TiC.PROPERTY_ICON:
            notificationBuilder
                    .setSmallIcon(TiUIHelper.getResourceId(newValue, this));
            mProcessUpdateFlags |= TIFLAG_NEEDS_UPDATE;
            break;
        ...
        case TiC.PROPERTY_WHEN:
            if (newValue instanceof Date) {
                ...
                notificationBuilder.setWhen(
                        ((Double) TiConvert.toDouble(newValue)).longValue());
            }
            break;
        ...
        case TiC.PROPERTY_CONTENT_INTENT:
            notificationBuilder.setContentIntent(
                    PendingIntentProxy.fromObject(newValue).getPendingIntent());
            mProcessUpdateFlags |= TIFLAG_NEEDS_UPDATE;
            break;
...
//            break;
        ...
        case TiC.PROPERTY_VIBRATE_PATTERN:
            if (newValue instanceof Object[]) {
                ...
                for (int i = 0; i < pattern.length; i++) {
                    vibrate[i] = ((Double) TiConvert.toDouble(pattern[i]))
                            .longValue();
                }
        ...
        case TiC.PROPERTY_GROUP_KEY:
            notificationBuilder.setGroup(
                    TiConvert.toString(newValue, TiC.PROPERTY_GROUP_KEY));
            mProcessUpdateFlags |= TIFLAG_NEEDS_UPDATE;
            break;
    ...
    protected void checkLatestEventInfoProperties(KrollDict d) {
        if (d.containsKeyAndNotNull(TiC.PROPERTY_CONTENT_TITLE)
            ...
            String contentText = "";
            if (d.containsKeyAndNotNull(TiC.PROPERTY_CONTENT_TITLE)) {
                contentTitle = TiConvert.toString(d,
    ...
    @Kroll.method
    public void addAction(Object icon, String title,
        ...
        if (pendingIntent == null) {
            Log.e(TAG,
                    "a pending intent for the action button must be provided");
            return;
        }
        notificationBuilder.addAction(iconId, title,
