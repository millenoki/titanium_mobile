 ...
 */
function detect(paramsOrSearchPaths, config, logger, callback, bypassCache) {
	var params;
		...
		project: function (next) {
			...
			async.each(Object.keys(searchPaths), function (modulePath, cb) {
				...
				detectModules({
					...
					modulesDir: (/modules$/.test(modulePath))?modulePath:path.join(modulePath, 'modules'),
 ...
 */
function find(modulesOrParams, platforms, deployType, tiManifest, searchPaths, config, logger, callback, bypassCache) {
	var params,
		result = {
			found: [],
			missing: [],
