...
- (UIViewContentMode)contentModeForImageView
{
  ...
  if (((TiDimensionIsAuto(width) || TiDimensionIsAutoSize(width)) && (TiDimensionIsUndefined(constraints->left) || TiDimensionIsUndefined(constraints->right))) && ((TiDimensionIsAuto(height) || TiDimensionIsAutoSize(height)) && (TiDimensionIsUndefined(constraints->top) || TiDimensionIsUndefined(constraints->bottom)))) {
...
- (void)setImage_:(id)arg
{
  ...
  _needsSetImage = NO;
  if (_currentImageSource && [_currentImageSource isEqual:arg] && _currentImage)
    return;
  if ([[self viewProxy] _hasListeners:@"startload" checkParent:NO]) {
    [self.proxy fireEvent:@"startload"
               withObject:@{
                 @"image" : arg
               }
                propagate:NO
  ...
  if (_filterOptions) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
      if (imageSource != _currentImageSource)
        return;
      RELEASE_TO_NIL(_currentImage);
      _currentImage = [[TiImageHelper imageFiltered:image withOptions:_filterOptions] retain];
      TiThreadPerformOnMainThread(^{
