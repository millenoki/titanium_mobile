...
{
  if (CGSizeEqualToSize(visibleBounds.size, CGSizeZero))
    return;
#ifndef TI_USE_AUTOLAYOUT
  CGRect viewBounds;
  ...
  if (!overlayEnabled || !showPageControl) {
    if (verticalLayout) {
      if (pagingControlOnTop)
        viewBounds.origin = CGPointMake(pagingControlHeight, 0);
