      ...
      if (from) {
        ...
        for (NSString *key in [from allKeys]) {
          if (IS_OF_CLASS([animProxy bindingForKey:key], TiAnimatableProxy)) {
            ...
            if (value) {
              if (value)
                [(NSMutableDictionary *)result setObject:value forKey:key];
            } else {
