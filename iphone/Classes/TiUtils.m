...
+ (UIImage *)stretchedImage:(UIImage *)image withCap:(TiCap)cap
{
  ...
  NSInteger left = (TiDimensionIsAuto(cap.leftCap) || TiDimensionIsUndefined(cap.leftCap) || cap.leftCap.value == 0) ? maxWidth / 2 : TiDimensionCalculateValue(cap.leftCap, maxWidth);
  NSInteger top = (TiDimensionIsAuto(cap.topCap) || TiDimensionIsUndefined(cap.topCap) || cap.topCap.value == 0) ? maxHeight / 2 : TiDimensionCalculateValue(cap.topCap, maxHeight);
...
+ (NSString *)replacingStringsIn:(NSString *)string fromDictionary:(NSDictionary *)dict withPrefix:(NSString *)prefix
{
  NSMutableString *result = [string mutableCopy];
  [dict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
    [result replaceOccurrencesOfString:[prefix stringByAppendingString:key]
                            withString:[TiUtils stringValue:obj]
                               options:0
                                 range:NSMakeRange(0, [result length])];
