...
{
  if (layoutProperties.fullscreen == YES)
    return size;
...
- (void)determineSandboxBoundsForce
{
  ...
  if (!TiCGRectIsEmpty(view.bounds)) {
    ...
    if (!CGRectIsEmpty(bounds)) {
      ...
      [self setSandboxBounds:([self viewParent]).sandboxBounds];
  } else {
...
- (BOOL)viewReady
{
  return view != nil && CGRectIsNull([view layer].bounds) == NO &&
...
#endif
  [self initializeProperty:@"visible"
...
- (void)willEnqueue
{
  ...
  if (parentVisible && !hidden && (!allowContentChange || instantUpdates))
    return;
...
{
#ifndef TI_USE_AUTOLAYOUT
  if (layoutProperties.fullscreen)
    return NO;
  BOOL isAutoSize = NO;
...
{
  if ([self heightIsAutoFill])
    return YES;
  BOOL followsFillBehavior = TiDimensionIsAutoFill([self defaultAutoHeightBehavior:nil]);
  if (TiDimensionIsDip(layoutProperties.height) || TiDimensionIsPercent(layoutProperties.height) || (!followsFillBehavior && TiDimensionIsUndefined(layoutProperties.height)))
    return NO;
...
{
  if ([self widthIsAutoFill])
    return YES;
  BOOL followsFillBehavior = TiDimensionIsAutoFill([self defaultAutoWidthBehavior:nil]);
  if (TiDimensionIsDip(layoutProperties.width) || TiDimensionIsPercent(layoutProperties.width) || (!followsFillBehavior && TiDimensionIsUndefined(layoutProperties.width)))
    return NO;
  NSArray *subproxies = [self visibleChildren];
  for (TiViewProxy *child in subproxies) {
...
- (void)parentWillHide
{
  ...
  if (!parentVisible) { //Nothing to do here, we're already invisible here.
    return;
  }
  parentVisible = NO;
  if (!hidden) { //We should propagate this new status! Note this does not change the visible property.
...
- (CGRect)computeChildSandbox:(TiViewProxy *)child withBounds:(CGRect)bounds
{
  ...
  CGFloat offsety = fullscreen ? 0 : TiDimensionCalculateValue(childConstraint->top, boundingHeight) + TiDimensionCalculateValue(childConstraint->bottom, boundingHeight);
...
- (void)layoutChildren:(BOOL)optimize
{
  ...
  if (CGSizeEqualToSize([[self view] bounds].size, CGSizeZero))
    return;
