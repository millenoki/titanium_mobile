...
- (void)dealloc
{
  ...
  RELEASE_TO_NIL(keyboardActiveInput);
...
- (void)dismissKeyboardFromWindow:(id<TiWindowProtocol>)theWindow
{
  if (self.keyboardActiveInput && [[self.keyboardActiveInput superview] isKindOfClass:[TiUIView class]]) {
    ...
    if (keyboardInputProxy && [theWindow isKindOfClass:[TiParentingProxy class]] && ![(TiParentingProxy *)theWindow containsChild:keyboardInputProxy]) {
      return;
    }
  }
                                ...
                                completion:^{
                                  if (presenter == self) {
                                    if ([theController respondsToSelector:@selector(proxy)]) {
                                      ...
                                      if (keyboardFocusedProxy && [theProxy isKindOfClass:[TiParentingProxy class]] && ![(TiParentingProxy *)theProxy containsChild:keyboardFocusedProxy]) {
...
- (void)childOrientationControllerChangedFlags:(id<TiOrientationController>)orientationController
{
  ...
  if ([self presentedViewController] == nil || ([TiUtils isIOS8OrGreater] && [[self presentedViewController] isKindOfClass:[UIAlertController class]]) && isCurrentlyVisible) {
