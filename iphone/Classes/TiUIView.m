...
- (UIImage *)loadImage:(id)arg
{
  ...
  if ([result isKindOfClass:[UIImage class]])
    return result;
  else if ([result isKindOfClass:[TiSVGImage class]])
    return [((TiSVGImage *)result)fullImage];
  return nil;
}
...
- (void)setusePathAsBorder:(BOOL)value
{
  if (value != usePathAsBorder) {
    ...
    if (runningAnimation) {
      ...
      if (_bgLayer)
        [_bgLayer addAnimation:animation forKey:@"cornerRadius"];
    }
    self.layer.cornerRadius = radius;
    if (_bgLayer)
      _bgLayer.cornerRadius = radius;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
  ...
  if (touchView1 != touchView2 || (IS_OF_CLASS(gestureRecognizer, UISwipeGestureRecognizer) && IS_OF_CLASS(otherGestureRecognizer, UIPanGestureRecognizer))) {
    return NO;
  }
  return YES;
...
- (void)transitionFromView:(TiUIView *)viewOut toView:(TiUIView *)viewIn withTransition:(TiTransition *)transition animationBlock:(void (^)(void))animBlock completionBlock:(void (^)(void))block
{
  ...
  [TiTransitionHelper transitionFromView:viewOut
                                  toView:viewIn
                              insideView:self
                          withTransition:transition
                            prepareBlock:NULL
                          animationBlock:animBlock
                         completionBlock:^{
