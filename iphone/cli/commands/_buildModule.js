...
iOSModuleBuilder.prototype.initialize = function initialize() {
    this.moduleIdAsIdentifier = this.moduleId
			.replace(/[\s-]/g, '_')
			.replace(/_+/g, '_')
			.split(/\./)
			.map(function (s) {
...
iOSModuleBuilder.prototype.compileJSFiles = function compileJSFiles(jsFiles, next) {
    ...
    var useBabel = this.useBabel; //for now this breaks the source map ...
    async.eachSeries(jsFiles, function(info, next) {
        setImmediate(function() {
            ...
            const relPath = path.dirname(info.path);
            const destDir = path.join(this.buildAssetsDir, relPath)
            ...
            if (useBabel) {
                this.cli.createHook('build.ios.compileJsFile', this, function(from, to,
                    ...
                    babel.transformFile(from, {
                        ...
                        try {
                            // parse the AST
                            const r = jsanalyze.analyzeJs(transformed.code, {
                                minify: minifyJS
                            });
                        } catch (ex) {
                        ...
                        if (_this.jsFilesToEncrypt.indexOf(file) === -1) {
                            ...
                            // console.log('jsFilesToEncrypt', path.join(relPath, file));
				...
				if (this.jsFilesToEncrypt.indexOf(file) === -1) {
					this.jsFilesToEncrypt.push(path.join(relPath, file));
				}
				...
				if (!exists || contents !== fs.readFileSync(dest).toString()) {
					this.logger.debug(__(minifyJS ? 'Copying and minifying %s => %s' : 'Copying %s => %s', src.cyan,
						dest.cyan));
				...
				if (fs.existsSync(src + '.map')) {
					...
					var mapPath = path.join(this.buildAssetsDir, moduleId, path.relative(this.buildAssetsDir, destDir));
					fs.existsSync(mapPath) || wrench.mkdirSyncRecursive(mapPath);
		...
		titaniumPrepHook = this.cli.createHook('build.ios.titaniumprep', this, function (exe, args, opts, done) {
			...
			async.whilst(
				...
				function (cb) {
					...
					jsFilesToEncrypt.forEach(function (file) {
                        relativePaths.push(file.replace(/\./g, '_'));
					}, this);
	...
	const tasks = [
		...
		function (cb) {
			this.jsFilesToEncrypt.forEach(function (file) {
                // console.log('analyzeJsFile', file);
                const r = jsanalyze.analyzeJsFile(path.join(this.buildAssetsDir, file), {
                    minify: true
                });
	...
	try {
        ...
        if (fs.existsSync(path.join(this.buildAssetsDir, moduleId))) {
            this.dirWalker(path.join(this.buildAssetsDir, moduleId), function(file) {
                if (/\.js\.map$/.test(file)) {
                    dest.file(file, {
                        name: path.join(moduleFolder, 'assets', path.relative(this.buildAssetsDir, file))
                    });
                }
            }.bind(this));
        }
        ...
        // 9. module.xcconfig
        // 10. metadata.json
        dest.file(binarylibFile, {
            name: path.join(moduleFolder, binarylibName)
        });
        if (fs.existsSync(this.licenseFile)) {
		...
		if (fs.existsSync(this.moduleXcconfigFile)) {
            ...
            dest.append(contents, {
                name: path.join(moduleFolder, 'module.xcconfig')
            });
		}
