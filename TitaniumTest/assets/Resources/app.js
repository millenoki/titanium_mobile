...
tableView.addEventListener('touchstart', function(e)
{
    if(e.row)
    {
    	...
    	Ti.API.info('touchRowPoint ' + JSON.stringify(touchRowPoint));
    }
});
...
tableView.addEventListener('touchmove', function(e)
{
    if(touchRow)
    {
    	var delta = e.globalPoint.x - touchRowPoint.x;
    	if (delta > 5 && delta < touchDecaleMax) 
    	{
    		touchRow.container.backgroundColor = getColorForPercentage(delta/touchDecaleMax);
        	touchRow.container.transform  = Ti.UI.create2DMatrix().translate(e.globalPoint.x - touchRowPoint.x, 0);
    	}
    }
});
