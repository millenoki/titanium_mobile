...
function zip(folder, filename, next) {
	...
	exec(command + ' -9 -q -y -x *.git* *.DS_Store -r "' + path.join('..', path.basename(filename)) + '" *', {cwd: folder}, function(err, stdout, stderr) {
		if (err) {
			return next(err);
		}
...
Packager.prototype.generateManifestJSON = function (next) {
	...
	fs.writeJSON(path.join(this.zipSDKDir, 'manifest.json'), json, function() {
		fs.writeJSON(path.join(this.zipDir, 'ZIP_META_MANIFEST'), json, next);
	}.bind(this));
};
...
Packager.prototype.zip = function (next) {
	console.log('creating zip', this.zipDir, this.zipFile);
...
Packager.prototype.package = function (next) {
	...
	async.series([
		...
		function (cb) {
			if (fs.existsSync(path.join(this.outputDir, 'titanium.d.ts'))) {
				fs.copy(path.join(this.outputDir, 'titanium.d.ts'), path.join(this.zipSDKDir, 'titanium.d.ts'), cb);
			}
		}.bind(this),
		function (cb) {
			// Copy some root files, cli/, templates/, node_modules minus .bin sub-dir
			this.copy(['CREDITS', 'README.md', 'package.json', 'license.json', 'cli', 'templates'], cb);
		}.bind(this),
